...
$(function () {
  ...
  $(document).on('click', '.faq__question', function () {
    ...
    if (isOpen) {
      ...
      var scrollTo = itemTop - $(window).height() / 2 + $btn.outerHeight() / 2;
      $('html, body').stop(true).animate({ scrollTop: scrollTo }, 500, 'linear');
    }
  });

  $(document).on('click', '.faq__answer', function () {
    var $btn = $(this).prev('.faq__question');
    $btn.attr('aria-expanded', 'false');
    $(this).slideUp(280);
  });
